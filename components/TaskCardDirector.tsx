  ...
  return (
    <div className={`border rounded-xl p-5 shadow-sm ${songColorClasses(task.color_tag)}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-xs font-bold px-2 py-1 rounded-full bg-white/70 dark:bg-slate-800/70 text-slate-600 dark:text-slate-300 flex-shrink-0">
          {done}/{total}
        </span>
      </div>
      ...
      <div className="flex flex-col mt-3 pt-3 border-t border-slate-200/60 dark:border-slate-700/60">
        {task.task_assignments.map((a: any) => (
          <AssignmentGrader key={a.id} assignment={a} />
        ))}
        {total === 0 && <p className="text-xs text-slate-400 dark:text-slate-500">Sin integrantes asignados.</p>}
      </div>
    </div>
  );
}
