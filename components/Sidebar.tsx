...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-slate-200/80 z-40 flex items-center justify-between px-4 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm">
            E
          </div>
          <span className="font-bold text-slate-900 text-sm">Emmanuel</span>
        </div>
        <button
      ...
      <aside
        className={`fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200/80 z-50 flex flex-col justify-between shadow-sm transition-transform duration-300 ease-in-out
        ${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
      >
        <div className="flex flex-col overflow-y-auto">
          <div className="h-16 md:h-20 px-4 flex items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/50">
            ...
            <button
              ...
              className="md:hidden p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 flex-shrink-0"
            >
